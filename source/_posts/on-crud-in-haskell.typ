...
这里所谓的“项目架构”是个狭义概念。谈到 API server，那肯定是由 Model 和 Controller 两大部分构成。其中，Model 的实质是在造/解释一个 eDSL，而 Controller 是在使用这个 eDSL。那么我们就有两种选择构造 eDSL：Free Functor 和 Tagless Final。本来我是想用 #link("https://hackage.haskell.org/package/free")[free] 的，但是这个库实在是太简陋，不适合工程，而 #link("https://hackage.haskell.org/package/polysemy")[polysemy] 的 example 里面充斥着各种类型和不知道功能的 Template Haskell，我甚至都没有看明白... 而众所周知，由于 $n^2$ 实例的原因， #link("https://hackage.haskell.org/package/mtl")[mtl] 对于自定义新的 Monad Transformer 非常不友好。最后我选择了 #link("https://hackage.haskell.org/package/fused-effects")[fused-effects] 这个号称拥有 Algebraic Effects 的库。
...
到此为止 Opaleye 都可以说是出众的，然而越过了这个类型安全的极限，就可以感到 Opaleye 在摆烂。Opaleye 不能 insert selected result，没有 on conflict update，更关键的是这些缺失的功能还难以自己实现。无奈我只好转向 beam。不过 Opaleye 在 0.8 版本把原本的`DoNothing`构造器隐藏了，可能是准备开始做 on conflict update的支持了，对此我表示十分期待。
