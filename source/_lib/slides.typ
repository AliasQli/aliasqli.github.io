...
#let alternatives(start: 1, lasting: -1, position: bottom + left, ..children) = {
    style(styles => {
        ...
        for (idx, child) in children.pos().enumerate() {
            let visible-subslides = if (idx == children.pos().len() - 1 and lasting != none) {
                if (lasting < 0) {
                    (beginning: start + idx)
                } else {
                    (beginning: start + idx, until: start + idx + lasting)
                }
            } else {
                start + idx
            }
