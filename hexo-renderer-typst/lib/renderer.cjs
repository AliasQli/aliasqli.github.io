          ...
          .then(async () => {
            ...
            const postFixup = () => {
              appElem.querySelectorAll('a[target="_blank"]').forEach((a) => {
                ...
                if (!/(^([a-zA-Z]+:)?\\/\\/|download)/.test(linkTo)) {
